...
@Component({
  selector: 'app-favoritos1',
  templateUrl: './favoritos1.component.html',
  styleUrls: ['./favoritos1.component.scss']
})
export class Favoritos1Component implements OnInit {
  ...
  onLoginGoogle() {
    this.loginGoogleUserFavorito().then((user) => {
      const direccion = 'Marcos Paz 700 (favoritos1)';
      const telefono = '460878 (favoritos1)';
      return this.updateUsuario(user.uid, user.displayName, user.email, user.photoURL, direccion, telefono);
    })
    .then((usuar) => {
      this.loginThen(usuar);
    });
    return true;
  }
